...
## About
Record labels earn profits by investing in an artist’s release cycle and then taking a stake in the revenue generated by the artist’s album. Whether the record label turns a profit is based on the album’s sales, which is in turn based on how popular the songs in the album are. When it comes to signing a new artist, record labels are forced to predict how well they think the album will sell. This becomes difficult since the quality of a song are qualitative and subjective in nature. Our goal with this project was to find out whether we could quantitatively predict the popularity of a song instead. 

Thus, this SC1015 Mini-Project focuses on finding what makes a song popular based on many variables. Three datasets are extracted from [Spotify’s API](https://developer.spotify.com/documentation/web-api/) and [Kaggle](https://www.kaggle.com/datasets/neisse/scrapped-lyrics-from-6-genres?resource=download) respectively, and then merged together. For detailed walkthrough, please view the source code in order from:
